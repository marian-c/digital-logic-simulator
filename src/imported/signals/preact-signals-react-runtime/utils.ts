import { Fragment, createElement, useMemo, type JSX } from 'react';
import type { ReadonlySignal, Signal } from '@/imported/signals/preact-signals-core';
import { useSignal, useSignals } from '@/imported/signals/preact-signals-react-runtime/index';
...
export function For<T>(props: ForProps<T>): JSX.Element | null {
  ...
  let list = ((typeof props.each === 'function' ? props.each() : props.each) as Signal<Array<T>>)
    .value;
