...
export default function useStorage<T extends string | number | boolean | object | null>(
  key: string,
  defaultValue?: T,
  getStorage: () => Storage | undefined = () => (isBrowser ? sessionStorage : undefined),
  ...
  useDeepCompareEffect(() => {
    const data =
      (storageValue ? (isFunction(storageValue) ? storageValue() : storageValue) : defaultValue) ??
      null;
  ...
  const updateState: Dispatch<SetStateAction<T | null>> = useEvent((valOrFunc) => {
