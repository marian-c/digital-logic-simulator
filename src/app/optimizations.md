...
- "4bit register" loses state when refreshing even though we store everything in local storage
...
- Simulation parts might be missing, if it errored mid way, ui should show this grayed out
- loading only cleans up, need one first simulation pass
- data is getting kinda big
  - use json files so TS does not waste time on it
  - use indexedDb instead of local storage
- reset to center button
- simulation data, at lease for the purposes of storing it, should be less redundant - only outputs can be stored; also inputs in addition data can be skipped
- button/shortcut to duplicate the current selected box
- quick box selector overlay
- move box from the keyboard
- pan sketch with arrow keys
- pan sketch with mouse click and drag, but differentiate from multi selection
- when rendering the subchips, the inputs and outputs should be in order of y coordinate
- don't simulate when switching sketches if it's been simulated before, we're loosing state
